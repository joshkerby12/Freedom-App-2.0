...
import { getSuppliers } from "@/lib/catalog/supplier-service";
...
export default async function SupplierListPage({ searchParams }: SupplierListPageProps) {
  ...
  return (
    <section className="space-y-4">
      {saved ? <p className="auth-feedback-success">Supplier deleted.</p> : null}
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Manage suppliers, contact info, and active locations.
            </CardDescription>
          </div>
          <Link
              ...
              <table className="min-w-full border-separate border-spacing-y-2">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-[0.08em] text-[var(--text-secondary)]">
                    ...
                    <th className="px-3 py-1">Locations</th>
                    <th className="px-3 py-1">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {suppliers.map((supplier) => (
                    <tr key={supplier.id} className="rounded-lg bg-[var(--surface)] shadow-[var(--shadow-card)]">
                      <td className="rounded-l-lg px-3 py-3 text-sm font-semibold text-[var(--text-primary)]">
                        <Link href={`/catalog/suppliers/${supplier.id}`} className="hover:underline">
                          {supplier.name}
                        </Link>
                      </td>
